import os
import pyvcad as pv
from pyvcad_rendering import Render_Sweep_Animation
...
if __name__ == "__main__":
    ...
    # We output to a temporary directory for automated testing
    out_dir = os.path.join(os.path.dirname(__file__), "sweep_frames_test")
    ...
    print(f"Rendering sweep animation to {out_dir} ...")
    ...
    Render_Sweep_Animation(
        ...
        duration=3.0,                # Total animation length in seconds for one pass
        fps=30,                      # Frames per second (default)
        loop=True,                   # Loop back forth (forward then back to start)
        pause_seconds=0.5,           # Pause at the beginning and end
        settings=settings,
