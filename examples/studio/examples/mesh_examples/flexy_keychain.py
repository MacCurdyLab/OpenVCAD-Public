...
blank = pv.Mesh(pv.resources_path() + "examples/data/3d_models/keychain_blank.3mf", yellow)

# Apply mechanical properties to the blank (soft to hard)
graded_blank = pv.FGrade(["min(max((-x/80)+0.835,0.5),1)",
                          "min(max((x/80)+0.165,0),0.5)"],
                         [white, black], True,
                         blank)
...
text = pv.Scale(1,2,1, 
                pv.Rotate(0,0,180, pv.Vec3(0,0,0),
                          pv.Mesh(pv.resources_path() + "examples/data/3d_models/openvcad_text.stl", yellow)
                          )
                )
